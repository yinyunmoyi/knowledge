...
图中的多个FD就是多个套接字，Redis调用epoll机制让内核监听这些套接字，此时线程不会阻塞在某一个特定的套接字上，也不会阻塞在某个特定的客户端请求上，所以Redis可以同时和多个客户端连接并处理请求，从而提升并发性。
...
一个 CPU 处理器中一般有多个运行核心，我们把一个运行核心称为一个物理核，每个物理核都可以运行应用程序。每个物理核都拥有私有的一级缓存（Level 1 cache，简称 L1 cache），包括一级指令缓存和一级数据缓存，以及私有的二级缓存（Level 2 cache，简称 L2 cache）。物理核的私有缓存，它其实是指缓存空间只能被当前的这个物理核使用，其他的物理核无法对这个核的缓存空间进行数据存取。
...
![QQ图片20221108224338](QQ图片20221108224338.png)
...
![QQ图片20221108224515](QQ图片20221108224515.png)
...
一个实际的调优案例：项目需求是要对 Redis 的 99% 尾延迟进行优化（把所有请求的处理延迟从小到大排个序，99% 的请求延迟小于的值就是 99% 尾延迟。）

检测了 Redis 实例运行时的服务器 CPU 的状态指标值，会发现CPU的context switch 次数比较多。context switch 是指线程的上下文切换，这里的上下文就是线程的运行时信息。在 CPU 多核的环境中，一个线程先在一个 CPU 核上运行，之后又切换到另一个 CPU 核上运行，这时就会发生 context switch。
...
解决方案是：避免 Redis 总是在不同 CPU 核上来回调度执行，让一个 Redis 实例固定运行在一个 CPU 核上。我们可以使用taskset 命令把一个程序绑定在一个核上运行。比如说，我们执行下面的命令，就把 Redis 实例绑在了 0 号核上，其中，“-c”选项用于设置要绑定的核编号。
...
在NUMA架构下，需要注意，应该将网络中断处理程序和Redis实例绑在同一个CPU Socket上，这样Redis实例就可以直接从本地内存读取网络数据了：

![QQ图片20221108225909](QQ图片20221108225909.png)
...
![QQ图片20221108230030](QQ图片20221108230030.png)

适合将切片集群的不同节点绑定在不同的Socket上运行，因为不同实例间不会使用共享内存空间。

绑核也是由风险的：当我们把 Redis 实例绑到一个 CPU 逻辑核上时，就会导致子进程、后台线程和 Redis 主线程竞争 CPU 资源（具体来说除了主线程，Redis 还有用于 RDB 和AOF 重写的子进程，以及 4.0 版本之后提供的用于惰性删除的后台线程。），一旦子进程或后台线程占用 CPU 时，主线程就会被阻塞，导致Redis 请求延迟增加。

可以采用两种解决方案来解决这个问题：

* 一个 Redis 实例对应绑一个物理核：在给 Redis 实例绑核时，我们不要把一个实例和一个逻辑核绑定，而要和一个物理核绑定，也就是说，把一个物理核的 2 个逻辑核都用上。和只绑一个逻辑核相比，把 Redis 实例和物理核绑定，可以让主线程、子进程、后台线程共享使用 2 个逻辑核，可以在一定程度上缓解 CPU 资源竞争。但是，因为只用了 2 个逻辑核，它们相互之间的 CPU 竞争仍然还会存在。
* 优化 Redis 源码：这个方案就是通过修改 Redis 源码，把子进程和后台线程绑到不同的 CPU 核上。
...
高性能、高并发的原因：基于内存、高效的I/O线程模型、高效的数据结构

一般像 MySQL 这类的数据库的 QPS 大概都在 1w 左右（4 核 8g） ，但是使用 Redis 缓存之后很容易达到 10w+，甚至最高能达到 30w+（就单机 Redis 的情况，Redis 集群的话会更高）

2、降低Redis数据在网络上传输的时间，是优化Redis性能很重要的一个方面，应该采取以下措施：

- 使用一些Redis提供的批量操作命令，如MGET、MSET等。它们是原子性的，如果在分片集群的场景下，原子性就无法保证了。
- 使用pipeline将一组命令一次性提交到Redis服务器执行，它不能保证原子性，而且不适用于一组有相互依赖关系的命令执行

上面这两种方式，在Redis Cluster 上使用会存在一些小问题：当操作的key不在一个节点时，还是会多很多网络传输和汇总操作

3、使用Redis需要注意哪些地方？

要注意的点：

- Redis 是使用单线程读写数据，bigkey 的读写操作会阻塞线程，降低 Redis 的处理效率。避免使用bigkey，对于值大小本身就很大的情况，可以压缩后存入，在业务层控制它的大小；对于集合元素很多的情况，可以将一个大集合拆分成多个小集合
- 建议把不同的业务数据放到不同的 Redis 实例中，它们的淘汰规则是不同的，应该避免相互干扰
- 禁用一些对Redis影响很大的命令，如KEYS、FLUSHALL、FLUSHDB、MONITOR
- 慎用Redis执行大数据量的聚合
...
- Redis6.0采用多个 IO 线程来处理网络请求（默认是禁用的），而对于读写命令，Redis 仍然使用单线程来处理
- 支持客户端缓存功能，可以直接把读取的数据缓存在本地，快速在本地读取数据
- 支持更细粒度的访问权限控制，支持不同用户设置命令操作的权限
- 实现RESP 3 通信协议

Redis 7.0新特性：

- AOF文件由一个变成了多个，分为基本文件和增量文件
- 支持限制客户端的内存使用量
- 用listpack代替ziplist

等等

5、缓存雪崩和缓存击穿的区别：缓存雪崩导致的原因是缓存中的大量或者所有数据失效，缓存击穿导致的原因主要是某个热点数据不存在与缓存中（通常是因为缓存中的那份数据已经过期）

6、Redis性能问题可能的原因：

- 使用复杂度过高的命令、或者返回了大量数据。可以通过查看慢查询日志来发现
- 操作bigkey
- 设置大量 key 集中过期。Redis 的过期数据采用被动过期 + 主动过期两种策略，被动过期就是访问key时判断key是否过期，若过期就会删除；主动过期是每隔 100 毫秒就随机取出一些key判断是否过期，如果过期key的比例超过了 25%，则继续重复此过程，这个定时任务是在Redis主线程中进行的，所以一旦大量key集中过期就会拖慢Redis主线程
- 实例内存达到上限，当 Redis 内存达到 maxmemory 后，每次写入新的数据之前，Redis 必须先从实例中踢出一部分数据，让整个实例的内存维持在 maxmemory 之下，然后才能把新数据写进来，这个操作也会增加响应延迟
- fork耗时：开启了后台 RDB 和 AOF rewrite 后都会使用fork创建子进程来进行持久化，fork时主进程需要拷贝自己的内存页表给子进程，如果这个实例很大，那么这个拷贝的过程也会比较耗时，这可能造成Redis实例阻塞。如果Redis实例部署在虚拟机上的时候，fork会更耗时
- 操作系统开启内存大页机制也会导致性能问题。内存大页就是常规的内存页从4K变成了2M，由于执行RDB时此时主进程依然可以写入数据，这时采取的是写时复制，主进程一旦有数据需要修改，Redis 并不会直接修改现有内存中的数据，而是先将这块内存数据拷贝出来，再修改这块新内存的数据，内存大页会导致拷贝阶段申请内存页的耗时增加。如果这个写请求操作的是一个 bigkey，那主进程在拷贝这个 bigkey 内存块时，一次申请的内存会更大，时间也会更久。
- AOF在将内存中的AOF刷新到磁盘的过程中，如果此时磁盘的 IO 负载很高，那这个后台线程在执行刷盘操作（fsync系统调用）时就会被阻塞住，这时主线程触发的任何刷盘都可能因此阻塞
- 使用swap导致性能下降
- 当Redis实例运行较长时间时，内存碎片率会越来越高，Redis4.0以下只能通过重启解决，Redis4.0推出了自动碎片整理功能，但是它也可能导致性能下降，因为Redis 的碎片整理工作是也在主线程中执行的，当其进行碎片整理时，必然会消耗 CPU 资源，产生更多的耗时，从而影响到客户端的请求

7、Redis被当做纯缓存使用时，可以仅在slave 节点低峰期执行 RDB 备份，不做AOF，这样就可以极大程度上避免RDB和AOF带来的不良影响

8、缓存读写策略详解：

- Cache Aside Pattern旁路缓存模式：服务端需要同时维系 db 和 cache，并且是以 db 的结果为准
- Read/Write Through Pattern读写穿透：服务端把 cache 视为主要数据存储，从中读取数据并将数据写入其中。cache 服务负责将此数据读取和写入 db，从而减轻了应用程序的职责
- Write Behind Pattern异步缓存写入：Write Behind Pattern 和 Read/Write Through Pattern 很相似，两者都是由 cache 服务来负责 cache 和 db 的读写。但是，两个又有很大的不同：Read/Write Through 是同步更新 cache 和 db，而 Write Behind 则是只更新缓存，不直接更新 db，而是改为异步批量的方式来更新 db。Write Behind Pattern 下 db 的写性能非常高，非常适合一些数据经常变化又对数据一致性要求没那么高的场景，MySQL 的 Innodb Buffer Pool 机制用到了这种策略

9、Cache Aside 策略适合读多写少的场景，不适合写多的场景。因为当写入比较频繁时，缓存中的数据会被频繁地清理，这样会对缓存的命中率有一些影响。如果业务对缓存命中率有严格的要求，那么可以考虑两种解决方案：

- 更新锁/常驻更新缓存线程：一种做法是在更新数据时也更新缓存，只是在更新缓存前先加一个分布式锁，因为这样在同一时间只允许一个线程更新缓存，就不会产生并发问题了。当然这么做对于写入的性能会有一些影响；
...
- 给数据的过期时间加上一个随机数，避免大量数据在同一时间过期
- 使用更新锁，当某个线程没有命中缓存时加锁，然后再去构建缓存，等构建完成后释放锁。未能获取互斥锁的请求，要么等待锁释放后重新读取缓存，要么就返回空值或者默认值。
- 所有缓存不设置有效期，将更新缓存的工作交给后台线程定时更新。它的缺点是当缓存被淘汰后，在定时更新前，业务线程读取缓存为空。可以将定时更新间隔取的比较短，或者在业务侧发现缓存数据被淘汰后，通过消息队列发送一条消息通知后台线程更新缓存
- 提前预热
...
1、Redis Set还提供了随机存取的命令，可以指定随机获取一个元素或者多个元素，适可以用于抽奖场景

2、info memory命令可以查看Redis内存碎片率，Redis4.0-RC3 版本以后自带了内存整理，可以避免内存碎片率过大的问题，可以设置参数限制内存碎片整理占用的CPU时间比例，以免影响正常程序。重启也可以对内存碎片进行整理，可以将碎片率过高的主节点转换为从节点，以便进行安全重启。

3、redis哈希表触发扩容的时机，以下任意一个条件被满足：

1）服务器目前没有在执行BGSAVE命令或者BGREWRITEAOF命令，并且哈希表的负载因子大于等于1。
2）服务器目前正在执行BGSAVE命令或者BGREWRITEAOF命令，并且哈希表的负载因子大于等于5。

BGSAVE：生成RDB；BGREWRITEAOF：执行AOF重写

当哈希表的负载因子小于0.1时，程序自动开始对哈希表执行收缩操作

4、SDS：简单动态字符串

在Redis3.0版本，它由三部分组成：保存的有效数据长度len 4字节、未使用的字节数量free 4字节、用于保存字符串的字节数组（以空字符\0结尾，它是ascii码值为0的字符，len不会包括这个字符）

SDS字节数组中的字符串以\0结尾，这样定义就和C字符串很类似，可以直接重用一部分C字符串库里面的函数

上面定义的字符串有一个不合适的地方是：即使字符串存储的数据量很少，也要用8个字节来代表它的长度

在Redis6.0版本（3.2之后），为了节省存储空间，将SDS细分为五种类型，类型标识占1个字节：

- 短字符串(长度小于32)：由类型标识（低3位存储类型, 高5位存储有效数据长度）、字节数组组成。此时它最多能装32位，所以free=32-len
- 长度占1字节：由类型标识（低3位存储类型, 高5位预留）、1字节的分配长度alloc、1字节的有效数据长度、字节数组组成
- 长度占2字节：由类型标识（低3位存储类型, 高5位预留）、2字节的分配长度alloc、2字节的有效数据长度、字节数组组成
- 长度占4字节：由类型标识（低3位存储类型, 高5位预留）、4字节的分配长度alloc、4字节的有效数据长度、字节数组组成
- 长度占8字节：由类型标识（低3位存储类型, 高5位预留）、8字节的分配长度alloc、8字节的有效数据长度、字节数组组成

它和C字符串的对比：

- C字符串不记录长度信息，所以为了获取字符串长度就必须遍历整个字符串；SDS可以直接根据字段获取字符串长度
- C字符串不记录长度信息可能会导致缓冲区溢出的情况出现，比如在没有考虑分配空间的情况下对字符串进行拼接操作，新生成的字符串可能很大，大到覆盖了周围的有用数据。而SDS在修改时，首先会检查SDS的空间是否满足修改的要求，如果不满足的话会进行空间扩展，不会有溢出问题出现。
- 内存分配策略不同。C的字符串每次改变时，都会对保存字符串的数组进行一次内存重分配操作，非常耗时，如果是增长字符串就是重分配来扩展空间，这个过程可能产生缓冲区溢出；如果是缩短字符串就是重分配来缩小空间，如果不释放空间就会产生内存泄露。而SDS在分配的时候会预分配一部分空间，以便未来增长不会导致重分配，缩短字符串的时候也不会立即释放内存，而是使用free字段将其记录下来，以备将来使用或者过段时间惰性删除
...
5、Redis String类型的key和value的限制均为512M，它其实就是2的32次方，最初是因为形容数组有效长度的len字段是4字节int，后续Redis6依然在源码层面有该限制。

6、Redis是可以存入空值的：SET mykey nil
...
- 发生故障恢复后，由于Redis数据是不可靠的（落盘丢数据、异步同步数据），所以不能保证消息至少被消费一次

Redis做队列的好处：相同成本下吞吐更高、延时更低、应用需要一个消息服务但又不想额外引入一堆依赖

8、在 Redis 4.0 之后的版本，在执行一些命令时就会使用主处理线程之外的其他线程，例如 `UNLINK`、`FLUSHALL ASYNC`、`FLUSHDB ASYNC` 等非阻塞的删除操作。

9、Redis 通过一个叫做过期字典（可以看作是 hash 表）来保存数据过期的时间。过期字典的键指向 Redis 数据库中的某个 key(键)，过期字典的值是一个 long long 类型的整数，这个整数保存了 key 所指向的数据库键的过期时间（毫秒精度的 UNIX 时间戳）

当对一个 key 设置了过期时间时，Redis 会把该 key 带上过期时间存储到一个过期字典（expires dict）中，也就是说「过期字典」保存了数据库中所有 key 的过期时间。每次查询一个key的时候，都会查询过期字典，来判断key是否过期。

Redis过期删除策略：惰性删除+定期删除

- 惰性删除：不主动删除过期键，每次从数据库访问 key 时，都检测 key 是否过期，如果过期则删除该 key
- 定期删除：每隔一段时间「随机」从数据库中取出一定数量的 key 进行检查，并删除其中的过期key。一般是取20个key检查是否过期，并删除已经过期的key，如果本轮已经过期key的数量超过5个，也就是25%，那么就继续重复取，继续这个循环过程。为了避免影响主线程，这个过程不会超过25ms

从库的过期键处理依靠主服务器控制，主库在 key 到期时，会在 AOF 文件里增加一条 del 指令，同步到所有的从库，从库通过执行这条 del 指令来删除过期的 key

10、Redis有一些命令时间复杂度可能在ON以上的命令：

- ZRANGE/ZREVRANGE：返回指定 Sorted Set 中指定排名范围内的所有元素。时间复杂度为 O(log(n)+m)，当m很大时执行耗时很长
- ZREMRANGEBYRANK/ZREMRANGEBYSCORE：移除 Sorted Set 中指定排名范围/指定 score 范围内的所有元素。时间复杂度为 O(log(n)+m)，当m很大时执行耗时很长

11、Redis 4.0引入了lazy-free 特性，指Redis 采用异步方式延迟释放 key 使用的内存（适用于处理过期key，同样适用于处理内存淘汰的key，它们两个是不同的lazy free选项），将该操作交给单独的子线程处理，避免阻塞主线程。它可以缓解大量 key 集中过期的问题。

unlink命令的底层和lazy-free 特性很像，但并非每次unlink命令就一定启用lazy free，当释放key的代价较大的时候才会启用异步

12、Redis3.2后数据类型与数据结构的关系：

- String由SDS实现（不变）
- list由quicklist实现
- hash由listpack或者哈希表实现
- set由整数集合或者哈希表实现（不变）
- zset由listpack或者跳表实现
...
- 查找复杂度高，只能逐个遍历entry
- 存在连锁更新的问题，在压缩列表中，每个entry要保存前一个节点的长度（为了方便从后向前遍历），当节点更新时可能导致占用空间要重新分配，此时可能触发长度值占用空间发生变化，进而引发连锁更新，影响更新性能

quicklist结构：它整体上是一个双向链表，链表的每个节点又是一个压缩列表，通过控制每个链表节点中的压缩列表的大小或者元素个数，来规避连锁更新的问题。因为压缩列表元素越少或越小，连锁更新带来的影响就越小，从而提供了更好的访问性能

listpack结构：它和压缩列表类似，它的特点是每个节点不再包含前一个节点的长度了，解决了连锁更新的问题

## 线程

1、Redis为了提高网络I/O读写性能，引入了多线程，但是Redis 的多线程只是在网络数据的读写这类耗时操作上使用了，执行命令仍然是单线程顺序执行。可以通过配置开启多线程，但是官网描述开启多线程读并不能有太大提升，因此一般情况下并不建议开启
...
- 后台进程：AOF重写、生成RDB
- 专门用于刷AOF日志的线程bio_aof_fsync
- 专门用于惰性删除的线程bio_lazy_free
- 专门用于释放AOF或者RDB过程中产生的临时文件资源bio_close_file

## 原子性

1、Redis lua脚本也是非原子性的，lua脚本若中途出错，之前已经成功执行的命令是不会回滚的。严格来说，Redis所有命令都是非原子性的，若有某个观点认为Redis命令是原子性的，那他指的应该是不会被打断、会执行完，但是它不能提供类似数据库的回滚机制。

2、Lua脚本的优点缺点：

- 优点：一段 Lua 脚本可以视作一条命令执行，中间不会有其他命令同时执行，可以看作是原子操作
- 缺点：执行到一半的Lua脚本如果中途结束，之前执行过的写操作也不会撤销，它不能实现类似数据库回滚的原子性。而且在Redis Cluster 下原子操作也无法保证了，因为操作的key可能是在不同的hash槽中

## bigkey&hotkey

1、识别bigkey的方法：

- 使用 Redis 自带的 --bigkeys 参数来查找，它对性能有影响，而且只能找到每种数据结构最大的key
- 自己用SCAN扫描
- 借助工具分析RDB文件
...
- 采用合适的数据结构，例如用bitmap、HyperLogLog

3、识别hotkey的方法：

- 使用 Redis 自带的 --hotkeys 参数来查找，使用该方案的前提条件是 Redis Server 的 maxmemory-policy 参数设置为 LFU 算法
- 使用MONITOR命令实时查看Redis执行的所有操作，它对性能的影响很大
- 业务侧进行统计、改写Redis客户端加入统计功能、改写Redis源码加入统计功能。可以用hash结构来保存每个key的访问频率，当key数量特别多，访问量特别大的时候，每次以万分之一的概率去记录它的次数，如果命中了就将次数+1，否则就直接丢掉。这样可以避免大量低频key的存储，因为我们只关心相对排名。

4、处理hotkey的方法：读写分离、使用Redis Cluster、服务层二级缓存（可以结合hotkey检测，检测到是热key就将其放入服务的内存中）

5、Redis大key对持久化的影响：

- 当使用 Always 策略的时候，如果写入是一个大 Key，主线程在执行 fsync() 函数的时候，阻塞的时间会比较久，因为当写入的数据量很大的时候，数据同步到硬盘这个过程是很耗时的
- AOF重写或者RDB时，如果操作大key，会触发写时复制，拷贝内存页时会阻塞一段时间

大 key 除了会影响持久化之外，还会有以下的影响：

- 客户端超时阻塞。由于 Redis 执行命令是单线程处理，然后在操作大 key 时会比较耗时，那么就会阻塞 Redis，从客户端这一视角看，就是很久很久都没有响应。
- 引发网络阻塞。每次获取大 key 产生的网络流量较大，如果一个 key 的大小是 1 MB，每秒访问量为 1000，那么每秒会产生 1000MB 的流量，这对于普通千兆网卡的服务器来说是灾难性的。
- del阻塞工作线程。如果使用 del 删除大 key 时，会阻塞工作线程，这样就没办法处理后续的命令。
...
1、数据库和缓存数据强一致场景方案：更新 db 的时候同样更新 cache，不过我们需要加一个锁/分布式锁来保证更新 cache 的时候不存在线程安全问题。

2、分布式集群中在多节点之间进行数据同步时，一个常见的做法是集中式发散消息，这种做法太依赖中心节点，有单点风险问题

Gossip协议是一种分散式发散消息的协议，它没有所谓的中心节点，而是每个节点周期性地随机的找一个节点互相同步彼此的消息，各节点的状态最终会保持一致。Redis Cluster 就是基于 Gossip 协议来实现集群中各个节点数据的最终一致性。

一些Gossip 协议的消息：MEET（用来将新节点添加到集群）、PING/PONG（用来交换各结点之间的信息）、FAIL（发现失联的节点）

有了 Redis Cluster 之后，不需要专门部署 Sentinel 集群服务了。Redis Cluster 相当于是内置了 Sentinel 机制，Redis Cluster 内部的各个 Redis 节点通过 Gossip 协议互相探测健康状态，在故障时可以自动切换。

Gossip的两种消息传播模式：

- 反熵：集群中的节点，每隔段时间就随机选择某个其他节点交换信息。为了实现确定的结果，一般会设计一个闭环，来指定哪些节点和哪些节点通信。它会传播节点所有的数据，它不适用于节点过多或者动态变化的场景
- 谣言传播：一个节点一旦有了新数据之后，就会变为活跃节点，活跃节点会周期性地联系其他节点向其发送新数据，直到所有的节点都存储了该新数据。它只传播节点变化的数据，它比较适用于节点数量比较多的情况

Gossip协议简单、方便扩容缩容，但是它需要多轮才能将消息传递到整个网络，中间会出现状态不一致的情况

3、缓存与数据的一致性解决方案：更新 DB，然后直接删除 cache。当更新数据库成功但是缓存删除失败时，解决方案：

- 让缓存失效时间变短
- 重试，如果 cache 服务当前不可用导致缓存删除失败的话，我们就隔一段时间进行重试，重试次数可以自己定。如果多次重试还是失败的话，我们可以把当前更新失败的 key 存入队列中，等缓存服务可用之后，再将缓存中对应的 key 删除即可

4、AOF重写时也会造成主线程阻塞，AOF重写的具体过程：

- fork 出一条子线程来将文件重写
- 在执行 BGREWRITEAOF 命令时，Redis 服务器会维护一个 AOF 重写缓冲区，该缓冲区会在子线程创建新 AOF 文件期间，记录服务器执行的所有写命令。当子线程完成创建新 AOF 文件的工作之后，服务器会将重写缓冲区中的所有内容追加到新 AOF 文件的末尾，使得新的 AOF 文件保存的数据库状态与现有的数据库状态一致。
- 最后，服务器用新的 AOF 文件替换旧的 AOF 文件，以此来完成 AOF 文件重写操作。

阻塞就是出现在第 2 步的过程中，将缓冲区中新数据写到新文件的过程中会产生阻塞

解决办法：noappendfsync-on-rewrite 设置为 yes，代表AOF重写时不进行fsync操作，不阻塞主线程，但是这可能丢失数据。关闭主动重写，在业务低峰期定时手动执行bgrewriteaof命令完成每日一次的AOF重写

5、Gossip协议：

在分布式系统中需要有一种数据同步机制，一个简单的方式是集中式发布消息，由主节点给其他节点发送消息，这种方法的缺陷是：节点多的时候不光同步消息的效率低，还太依赖与中心节点，存在单点风险问题。

Gossip协议是一种分散式发布消息的协议，它是一种去中心化的通信协议，Redis、Apache Cassandra、Consul都用到了这种协议。

以 Redis Cluster 为例说明 Gossip 协议的实际应用，集群中的每个节点基于Gossip 协议 来进行通信共享信息，每个 Redis 节点都维护了一份集群的状态信息。

Redis Cluster 的节点之间会相互发送多种 Gossip 消息：

- MEET：将新的节点添加到集群中
- PING/PONG：集群中的节点定时地向其他节点发送 PING 消息，来交换各个节点状态信息
- FAIL：节点 A 发现 B 节点 PFAIL ，并且在下线报告的有效期限内集群中半数以上的节点将 B 节点标记为 PFAIL，节点 A 就会向集群广播一条 FAIL 消息，通知其他节点将故障节点 B 标记为 FAIL

有了 Redis Cluster 之后，不需要专门部署 Sentinel 集群服务了。Redis Cluster 相当于是内置了 Sentinel 机制，Redis Cluster 内部的各个 Redis 节点通过 Gossip 协议互相探测健康状态，在故障时可以自动切换。

Gossip 设计了两种可能的消息传播模式：反熵（Anti-Entropy） 和 传谣（Rumor-Mongering）

- 反熵：熵可以理解为节点之间数据的混乱程度/差异性，反熵就是让多个节点的数据趋于同步，具体来说就是集群中的节点，每隔段时间就随机选择某个其他节点，然后通过互相交换自己的所有数据来消除两者之间的差异，实现数据的最终一致性。实际应用中并不会真的采用随机的方式，而是在一定的时间内实现各节点数据的最终一致性。当节点过多或者节点动态变化的话，这个方法就不适用了
- 谣言传播：一个节点一旦有了新数据之后，就会变为活跃节点，活跃节点会周期性地联系其他节点向其发送新数据，直到所有的节点都存储了该新数据。它比较适合节点数量比较多的情况，不过，这种模式下要尽量避免传播的信息包不能太大，避免网络消耗太大。

Gossip协议的优势和劣势：

优势：简单易懂、能容忍网络上节点的随意地增加或者减少，宕机或者重启（因为每个节点地位都是平等的）、同步数据速度相对较快（比一个主节点向其他节点传播信息要更快）

缺陷：消息需要通过多个传播的轮次才能传播到整个网络中，因此，必然会出现各节点状态不一致的情况，而且可能会有消息冗余

6、Redis的AOF日志设置为always时，就不会出现数据丢失吗？

答：数据也有可能丢失，因为每次执行客户端命令时并不会将命令直接写入AOF文件，而只是写入文件缓冲区中，所以还是可能出现数据丢失的情况

7、主从服务器在完成第一次同步后，就会基于长连接进行命令传播。从 Redis 2.8 开始，网络断开又恢复后，从主从服务器会采用增量复制的方式继续同步，也就是只会把网络断开期间主服务器接收到的写操作命令，同步给从服务器。

8、主从集群的心跳检查：

- Redis 主节点默认每隔 10 秒对从节点发送 ping 命令，判断从节点的存活性和连接状态，可通过参数repl-ping-slave-period控制发送频率
- Redis 从节点每隔 1 秒发送 replconf ack{offset} 命令，给主节点上报自身当前的复制偏移量，也是检测主从节点网络状态

9、Redis异步复制导致数据丢失：当主节点还没来得及将数据同步给从节点就发生了断电，就会导致数据丢失。此时可以考虑修改参数min-slaves-max-lag，例如将它配置为10s，表示所有的从节点数据复制和同步延迟超过10s则主节点拒绝接受写请求，这样就能把数据丢失控制在10s内。
...
1、为什么Redis Cluster是16384个槽

原因有两点：

- 16384正好是2的16次方，方便计算得到hash值后，快速得到key对应的槽位。由于通信协议的考虑，心跳包会带着节点负责的哈希槽的信息，所以设计的多一倍，心跳包就会大一倍，16384是一个合适的值
- 因为集群规模是有上限的，集群最多支持1000个实例

2、Redis cluster数据迁移过程：它是同步的

首先执行命令让源节点准备迁出槽的数据，然后获取要迁出key的数量，在源节点循环执行命令，如迁移key 0，key 1，等等，直到迁出完成，并在集群节点执行命令，通知它们槽已经分配给了目标节点。也可以使用redis-cli自动迁移槽

3、一个最基本的Redis Cluster的架构是这样的：至少需要3个master和3个slave，每个master必须有1个slave，这个slave和它对应的master之间是主从关系，slave实时同步master上的数据，这里的slave一般不会对外提供服务，它的作用是当master出现故障的时候替代它