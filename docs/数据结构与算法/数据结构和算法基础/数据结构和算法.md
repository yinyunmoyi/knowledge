...
有时检验滑动窗口是否满足要求，就需要遍历一遍窗口，此时可以尝试用一个变量来代表当前滑动窗口状态，省去对比的耗时。

例题1：leetcode438 找到字符串中所有字母异位词，找出cbaebabacd中所有abc的异位词。此题也是典型的滑动窗口解法，而且是固定长度窗口，这种类型的题目用一个变量来描述两个字符串之间的差异即可，窗口前进一步只需要维护这个变量即可，用一个数组代表窗口和p之间的差异：
...
例题2：leetcode76 最小覆盖子串，找出ADOBECODEBANC中可以覆盖ABC的最小子串，即BANC
...
用hash数组来统计每个字母，它并不是代表频次，而是代表当前窗口各字母的多余值，当值为负数时代表窗口需要该值，当值为正数时代表该值对于窗口来说是多余的，依次来控制窗口的移动。

滑动窗口基本的移动模式：向右探索至最大，然后开始缩小左边界，每向右前进一步，就尽可能的缩小左边界

右边界遍历到一个新的位置时，就给当前位置的值+1，代表窗口新增了一个元素，如果窗口值小于等于0则代表关键元素的频次发生了变化，所谓关键元素就是初始状态字符串t中含的元素，因为只有关键元素初始值是负数，才有可能在加1之后该值依然小于等于0，当cnt等于m时，代表关键元素已经不缺了，此时窗口就必然是合法的了。cnt只增不减，代表之后的窗口一直都是合法值。

当右边界扩充到极限，即cnt == m后就开始了左边界的缩小，它的规则是：当右边界划过某个值A，A就成了在满足要求窗口基础上的多余值，只有左边界指针对应的值是多余值的时候，左边界才会前进1，同时将hash数组减1
...
* 采用long，而不是int
* 任何计算结果后面，都要取余，得到一个相对较小的数字，以保持后续计算过程顺利。对于进制的N次方这种一计算就会溢出的数字，采用累加的方式进行，每次累计相乘都会取余
	...
	private static int[] getHelper(String str) {
        ...
        while (posi < dp.length) {
            if (arr[posi - 1] == arr[key]) {
                dp[posi++] = key + 1;
                key++;  // 为什么这里是key++，其实替换为key = dp[posi - 1]更好理解一些，这两个是一个意思
            } else if (key > 0) {
                key = dp[key];
            } else {
                dp[posi++] = 0;
...
![QQ图片20230713211833](QQ图片20230713211833.png)
...
* 初始start和end的位置
* while条件
* while循环中的if条件，及每个分支的逻辑

一般的解题步骤总结如下：

* 根据是否会漏掉结果，初步确定上面的几个要素，写出程序框架
* 检查程序高危点1：注意分支逻辑是start=mid或者end=mid的分支，这样的分支很容易造成死循环，因为它实际上没有缩小搜索范围。构造该分支死循环的场景，修正程序的逻辑
* 检查程序高危点2：当start和end相差1的时候，程序也很容易陷入死循环，因为if条件通常是start、mid和end相互判断，若三个数中存在相同的则为程序的边界，要特别注意。构造该情况下死循环的场景，修正程序的逻辑
* 检查程序高危点3：if条件未覆盖全部逻辑分支的时候，也就是仅仅只有else if，没有else的时候，要仔细考虑else是否漏掉什么场景，如果真的出现，应该怎样应对
...
```java
//这种缓存结构内部维护着两个hashMap（其实一个就可以）和一个双向链表
...
用Java自带的LinkedHashMap，重写removeEldestEntry方法设置自定义的淘汰逻辑：
...
LFU是一种缓存算法，它在缓存满了的时候，移除的是最不经常使用的项（使用频率最低的），如果两个键相同使用频率，则移除最久未使用的键值
...
class LFUCache {
  ...
  Map<Integer, Integer> keyToTimes;

  Map<Integer, Set<Integer>> keyToSets;
  ...
  public void put(int key, int value) {
    ...
    if (keyToVal.containsKey(key)) {
      ...
      keyToSets.putIfAbsent(1, new LinkedHashSet<>());
      keyToSets.get(1).add(key);

      if (capacity < keyToVal.size()) {
        int minKey = keyToSets.get(minTime).iterator().next();
        keyToVal.remove(minKey);
        keyToTimes.remove(minKey);
        keyToSets.get(minTime).remove(minKey);
        if (keyToSets.get(minTime).size() == 0) {
          keyToSets.remove(minTime);
        }
      }
      minTime = 1;
    }
  ...
  private void timesAdd(int key) {
    int times = keyToTimes.get(key);
    keyToTimes.put(key, times + 1);
    Set<Integer> sets = keyToSets.get(times);
    sets.remove(key);
    if (sets.isEmpty()) {
      keyToSets.remove(times);
    }
    if (times == minTime && sets.isEmpty()) {
      minTime++;
    }
    keyToSets.putIfAbsent(times + 1, new LinkedHashSet<>());
    keyToSets.get(times + 1).add(key);
  }
}
~~~
...
以leetcode470：用 Rand7() 实现 Rand10()来说明拒绝采样的思路

题目要求的结果是实现Rand10()，其实我们只需要等概率的生成 10 个不同的数即可，这里介绍几种思路：

* 利用两个 Rand7() 相乘，得到各数字的概率如下：

  ![QQ图片20230422112807](QQ图片20230422112807.png)

  只要从中挑选10个等概率的即可，例如挑选概率为2/49的十个数字，如果不是这十个数字就拒绝，进行下一次取样，直到命中这10个数字，然后建立到1-10的映射，就可以实现rand10了

  但是该方法拒绝的数字比较多，可能调用多次rand7都没有命中

* 利用两次rand7得到1-49之间的随机整数，拒绝最后9个数字，用前40个用来实现rand10，这样每一轮被拒绝的概率就只有9/49：
  ...
  注意：直接两个rand7相乘是无法得到均匀的1-49的，必须是先将其中一个变为0-x，再乘以常数a，才能得到0-xa的数字

* 调用两次rand7，第一次拒绝7，限定[1,6]，判断奇偶性，概率是1/2；第二次拒绝6和7，限定[1,5]，概率是1/5，这样就能组合出来1/10的概率了
...
辗转相除法是求最大公约数的经典方法，假设有a和b，那么ab的最大公约数为：
...
求到最大公约数，也自然能得到最小公倍数：最大公约数乘以最小公倍数=两数乘积

## 加权轮询算法

leetcode528：加权随机算法，前缀和+二分法

加权轮询算法是负载均衡法的一种。

算法逻辑具体如下：

* 算法开始时，为每个服务器分配一个初始权重，通常情况下，权重值可以根据服务器的性能、处理能力、连接数等因素进行设置。权重值越高，表示该服务器可以处理更多的请求。
* 当有新的请求到达时，负载均衡器会选择具有最高权重的服务器处理该请求。
* 选定服务器处理请求后，该服务器的权重会减去一-个事先设定好的值（一般是所有权重的最大公约数），以降低其权重，使得在下一次轮询中被选中的可能性减小。
* 当所有服务器的权重值都减为0时，它们将重新被赋予初始权重值，然后再次参与请求的分发。
* 重复上述步骤，直到服务终止或者负载均衡器收到终止信号。
