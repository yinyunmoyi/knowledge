...
1、Integer i1=40和Integer i2 = new Integer(40)两个对象是不相等的，第一个是自动装箱，从缓存中取的数据；第二个是创建了新的对象

2、泛型的一些限制：

- 只能声明不能实例化T类型变量
- 不能实例化泛型数组
- 不能使用instanceof和getClass进行类型判断
- 不能使用static修饰泛型变量

3、List<?>和List的区别：不能向List<?>中添加元素，会报错

4、打印的字符串为null，打印出来就是null，这是因为PrintStream在打印String类型时，对null值做了特殊处理。

同样的道理，在两个null字符串相加后值也是nullnull，因为StringBuilder的append方法也对它做了特殊处理。

5、String = "a"+"b"+"c"共创建了几个对象？答：一个，因为编译器有常量折叠这个优化

String s1="a"  String s2 = s1+"bb" 那么s2和"abb"是不相等的，因为无法在编译器确定s1的值，也就无法做常量折叠优化，但是若s1是final修饰的则可以

6、Java技术下有两种编译器：

- 把java文件转变为class文件的前端编译器
- 运行期把字节码转变成本地机器码的即时编译器JIT，或者提前将程序编译为机器码的提前编译器AOT，它们也叫后端编译器

一些前端编译器的优化：常量折叠，例如将1+1直接替换为2，等

7、分层编译

目前主流的商用Java虚拟机，如HotSpot，内部都同时包含解释器和编译器。它们各有优势：解释器可以首先发挥作用，省去编译的时间，立即运行。当程序启动后，随着时间的推移，编译器逐渐发挥作用，把越来越多的代码编译成本地代码，减少解释器的中间损耗，获得更高的执行效率。

HotSpot虚拟机中包含两个即时编译器：Client Compiler和Server Compiler。在分层编译出现之前，HotSpot虚拟机只能选择一个编译器来运行。

为了这程序启动速度和运行效率之间达到最佳平衡，HotSpot虚拟机加入了分层编译的功能，它将运行划分为不同的编译层次，其中包括：

- 第0层：程序纯解释执行，解释器不开启性能监控功能
- 第1层：使用客户端编译器将字节码编译为本地代码执行，进行简单可靠的稳定优化
- 第2、3层：仍然使用客户端编译器执行，逐步开启全部性能监控
- 第4层：使用服务端编译器将字节码编译为本地代码执行，它会启用更多编译耗时更长的优化

8、提前编译器目前的用处是减少程序启动时间和到达全速性能的预热时间，提前编译器的峰值性能可能是不如即时编译器的，因为即时编译器可以在运行期不断收集数据，这些数据在静态编译分析阶段是无法得到的，因此可以作出很多激进、极致的优化。

9、反射的原理是什么？

反射的原理是运行时加载类，得到JVM中的Class对象，对它进行解析，获取类的各种信息

10、多线程循环打印ABC

多个线程共用一个成员变量flag（不用加volatile），所有线程都在死循环中，死循环中线获取锁，线程1检查flag是不是A，若是退出while循环，打印A，然后notifyAll；若不是，调用wait方法阻塞

11、Lambda表达式和Stream的原理

Lambda表达式是语法糖

Stream的原理：每个步骤都生成1个Pipeline对象，遇到转换算子生成新的Pipeline对象，遇到终止操作就递归调用所有对象

Stream并行流的原理：Fork-Join框架

12、BigDecimal使用的注意事项：

- 不能使用构造方法传入一个double的方式初始化BigDecimal，它存在精度损失风险，应该使用BigDecimal.valueOf，或者入参为String的构造方法
- BigDecimal可以选择保留几位小数，divide要使用三个入参的，否则无法除尽时会报错
- BigDecimal等值比较应该使用compareTo，禁止使用equals，因为equals会同时比较直和精度，compareTo只是数值的比较

13、Arthas的原理

JDK5提供了一种高级特性Java执行容器，它可以构建一个独立于应用程序的代理程序，用来监测和协助运行在JVM上的程序，还能替换和修改某些类的定义，相当于提供了一种虚拟机级别支持的 AOP 实现方式
...
15、对于一个 32 位 Java 应用来说，用户进程空间是 4GB

16、为什么jdk9将String的底层实现由char数组替换为byte数组？因为byte占用的内存更少

17、Go因为强大的并发和网络能力，更适合开发一些网络服务、云应用程序

Go和Java的对比：

- Go有强大的并发模型，有协程的概念，能大大减少并发切换线程的性能开销；而Java没有协程的概念，依然采用内核级线程模型
- Go更擅长管理内存，它的垃圾收集速度比Java更快
- Java要考虑兼容性，所以它很多设计比较复杂，而Go是新语言，它的库要简单的多，因此性能会更好一些
- Go的代码更简洁，删除了不必要的括号和多余的符号
- Java必须运行在JVM上，而Go在编译器层面就可以实现跨平台，生成可以直接运行的可执行文件
- Go是新语言，没有Java那么强的生态